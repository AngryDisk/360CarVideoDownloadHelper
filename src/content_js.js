...
function whichTV(){
  var hostValue = window.location.host,returnValue;
  switch(hostValue){
  case "wap.che.360.cn":
    return "360";
  case "v.xiaoyi.com":
...
function realDownloadUrl() {
  ...
  return configsObj.playlist[0].url.replace(/http/, "https");
    case "v.xiaoyi.com":
      return $("#myVideo").attr("src").replace(/http/, "https");
    case "camera.leautolink.com":
      //需要先播放一下视频
      return $("#video_player").attr("src");
    case "qsurl.goluk.cn":
      return $("#wonvideo video").attr("src").replace(/http/, "https");
    case "cdn.static.ddpai.com":
      return $("video").attr("src").replace(/http/, "https");
      default:
